...
var signal = bytes.NewBuffer([]byte("fuck"))
...
func (fsrv *FileServer) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
  ...
  logger.Infof("fileserver, filename <%v>", filename)
  ...
  // // 当客户端要访问的资源在 MemoryStorage 中时，直接使用内存中的数据。如果要访问的资源
  // // 不在 MemoryStorage 中时，就调用系统 HTTP 库返回此资源
  // logger.Infof("fileserver: filename = <%v>, size = <%v>\n", filename, info.Size())
  // // 从内存区域直接取出要返回的数据
  // var result bool
  // if filename == "." {
  //   // 文件名为 <.> 是请求的是 index.html
  //   result = quic.GetMemoryStorage().Contains("index.html")
  //   // result = GetMemoryStorage().GetByURL("index.html")
  // } else {
  //   result = quic.GetMemoryStorage().Contains(filename)
  // }

  // if result {
  //   // 缓存命中
  //   logger.Infof("sending signal to reaplace url = <%v> with cached resource", filename)
  //   io.Copy(w, signal)
  //   return nil
  // }
  ...
  // logger.Infof("cache missed, key = <%v>", filename)
