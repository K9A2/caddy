...
# Mockgen refuses to generate mocks for core packages.
# This script copies the core directory and renames it to internalpackage.
# It also creages a public alias for private types.
# It then creates a mock for this public (alias) type.
# Afterwards, it corrects the import paths (replaces internalpackage back to core).
...
mv $TEMP_DIR/src/github.com/lucas-clemente/quic-go/core $TEMP_DIR/src/github.com/lucas-clemente/quic-go/internalpackage
find $TEMP_DIR -type f -name "*.go" -exec sed -i '' 's/core/internalpackage/g' {} \;
...
sed -i '' 's/internalpackage/core/g' $2
